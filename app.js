    ...
    <div class="event">
      ...
      <div class="join-card">
        <p class="contact-helper">
          Choisissez un seul moyen de contact : email <strong>ou</strong> SMS.
        </p>
        <input type="text" id="name-${ev.id}" placeholder="Nom">
        <input type="email" id="email-${ev.id}" placeholder="Email">
        <input type="tel" id="phone-${ev.id}" placeholder="Téléphone">
...
// ------------------ FORMULAIRE MAIL / SMS PAR ÉVÉNEMENT ------------------

function setupContactChoice() {
  ...
  cards.forEach((card) => {
    ...
    const btn = card.querySelector("button.btn-full");

    if (!btn) return;

    // état initial : bouton bloqué tant qu'aucun contact
    btn.disabled = true;
    ...
    function refresh() {
      ...
      if (emailInput) {
        ...
        emailInput.classList.remove("field-disabled");
      }
      if (phoneInput) {
        phoneInput.disabled = false;
        phoneInput.classList.remove("field-disabled");
      }
      ...
      if (email && !phone) {
        if (phoneInput) {
          ...
          phoneInput.classList.add("field-disabled");
        }
        btn.disabled = false;
      ...
      if (phone && !email) {
        if (emailInput) {
          ...
          emailInput.classList.add("field-disabled");
        }
        btn.disabled = false;
        btn.classList.remove("btn-disabled");
        return;
      ...
      // Si les deux sont remplis (cas rare), on garde le dernier tapé
      // et on vide l'autre (géré dans les listeners ci-dessous).
    }
    ...
    if (emailInput) {
      emailInput.addEventListener("input", () => {
        ...
        const phone = phoneInput ? phoneInput.value.trim() : "";
        if (email && phone && phoneInput) {
          // si l'utilisateur saisit un email alors qu'il y avait déjà un tel,
          // on garde l'email et on vide le téléphone
          phoneInput.value = "";
        }
        refresh();
      });
    ...
    if (phoneInput) {
      phoneInput.addEventListener("input", () => {
        ...
        const email = emailInput ? emailInput.value.trim() : "";
        if (phone && email && emailInput) {
          // si l'utilisateur saisit un tel alors qu'il y avait déjà un email,
          // on garde le téléphone et on vide l'email
          emailInput.value = "";
        }
        refresh();
      });
...
async function loadEvents() {
  ...
  // branche la logique mail / SMS sur les cartes fraichement rendues
  setupContactChoice();
}
...
async function joinEvent(eventId) {
  ...
  // Règle : exactement un moyen de contact (email OU téléphone)
  if (!email && !phone) {
    msg.textContent = "Merci de choisir un moyen de contact (email ou téléphone).";
    msg.style.color = "red";
    return;
  }
  if (email && phone) {
    msg.textContent =
      "Merci de renseigner soit l'email soit le téléphone, mais pas les deux.";
    msg.style.color = "red";
    return;
  }
  ...
  // on laisse Supabase Realtime + le rechargement forcer la vue à jour
  loadEvents();
}
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  // Rafraîchissement auto toutes les 30s en plus du realtime,
  // mais seulement quand l'onglet est visible (évite de spammer).
  setInterval(() => {
  ...
  // PWA : bouton global
  installBtnGlobal = document.getElementById("installPWA");
  if (installBtnGlobal) {
    installBtnGlobal.addEventListener("click", async () => {
      if (!deferredPrompt) return;
      deferredPrompt.prompt();
      const choice = await deferredPrompt.userChoice;
      console.log("Résultat installation :", choice);
      deferredPrompt = null;
      installBtnGlobal.classList.add("hidden");
    });
  }
});
...
window.addEventListener("beforeinstallprompt", (e) => {
  ...
  if (installBtnGlobal) {
    installBtnGlobal.classList.remove("hidden");
  }
